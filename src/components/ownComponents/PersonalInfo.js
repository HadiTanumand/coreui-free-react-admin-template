...
const PersonalInfo = (props) => {
  ...
  const [helpContractName, setHelpContractName] = useState(0)
  const [subcontract, setSubcontract] = useState(0)
  const [subTwocontract, setSubTowcontract] = useState(0)
          ...
          <CCardBody>
            ...
            <CForm className="row g-3 needs-validation">
              ...
              <CCol md={4}>
                ...
                >
                  {contractType[helpContractName].type.map((c, index) => {
                    return <option value={index}>{c.name}</option>
                  })}
                </CFormSelect>
              </CCol>
              <CCol md={4}>
                <CFormLabel htmlFor="validationServer03" required>
                  {subcontract == 2 && helpContractName != 2 ? 'آپشن' : ''}
                </CFormLabel>
                <CFormSelect
                  hidden={subcontract == 2 && helpContractName != 2 ? false : true}
                  ...
                  className="shadow"
                >
                  {contractType[helpContractName].type[subcontract].data.map((d, index) => {
      ...
      <CCol xs={12} className="shadow">
        <CCard className="mb-4">
          ...
          <CCardBody>
            <p className="text-medium-emphasis"></p>
            <CListGroup className="mt-4 col-lg-4 shadow">
              <CListGroupItem>شماره قرارداد: {props.formdata.contract_number}</CListGroupItem>
              <CListGroupItem> قرارداد: {props.formdata.contract_id}</CListGroupItem>
            </CListGroup>
