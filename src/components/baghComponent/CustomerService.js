...
const CustomerService = (props) => {
  ...
  function handleChangeMan(value) {
    let month = value.month.number<10 ? "0"+value.month.number.toString() : value.month.number; 
    let day = value.day<10 ? "0"+value.day.toString(): value.day ; 
    //تغییرات روی تاریخ رو اینجا اعمال کنید
    props.setformdata({
      ...props.formdata,
      manBirthdate: `${value.year}-${month}-${day}`,
    })
  }
  function handleChangeWoman(value) {
    //تغییرات روی تاریخ رو اینجا اعمال کنید
    // console.log(value)
    // console.log(value.month.number.toString())
    // console.log(value.day)
    let month = value.month.number<10 ? "0"+value.month.number.toString() : value.month.number; 
    let day = value.day<10 ? "0"+value.day.toString(): value.day ; 
    props.setformdata({
      ...props.formdata,
      womanBirthdate: `${value.year}-${month}-${day}`,
    })
  }
